...
class Agent(object):
    ...
    def temp(self, next_batch):
        return self.__policy(next_batch.float()).max(1).indices.view(32, 1)
    ...
    def learn(self, memory: ReplayMemory, batch_size: int) -> float:
        ...
        #double DQN
        #         next_action = self.temp(next_batch)
        #         values_next_tol = self.__target(next_batch.float()).gather(1, action_batch)
        #         values_next = torch.zeros([batch_size]).cuda()
        #         values_next = values_next_tol.gather(1,next_action).view(32)
